              ...
              return (
                <article key={project.id} className="editorial-card rounded-2xl p-6 sm:p-8">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <p className="rounded-full bg-primarySoft px-3 py-1 text-xs font-medium text-primaryStrong">{t(`projectsPage.items.${project.id}.role`)}</p>
                  </div>
                  <p className="mt-4 max-w-2xl text-sm leading-relaxed text-subtle">{t(`projectsPage.items.${project.id}.summary`)}</p>
                  <ul className="mt-5 flex flex-wrap gap-2" aria-label={t("projectsPage.stackLabel", { title })}>
